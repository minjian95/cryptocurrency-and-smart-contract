...
contract('MetaCoin', function(accounts) {
  ...
  it("should allow owner to mint coins", function() {
    //call the mint function on behalf of the owner and check that owner's account balance increased correctly
      ...
      return MetaCoin.deployed()
          ...
          .then(() => meta.getBalance.call(owner))
          .then(balance => {
  ...
  it("should not allow a non-owner address to mint coins", function() {
    //call the mint function on behalf of a non-owner address and check that the non-owner address balance stays the same
      ...
      let non_owner_starting_balance;
      let non_owner_ending_balance;

      return MetaCoin.deployed()
          .then(instance => {
              meta = instance;
              return meta.getBalance.call(non_owner)
          })
          .then(balance => {
              non_owner_starting_balance= balance.toNumber();
              return meta.mint(amount);
          })
          .then(() => meta.getBalance.call(non_owner))
          .then(balance => {
              non_owner_ending_balance = balance.toNumber();
              assert.equal(non_owner_starting_balance, non_owner_ending_balance, "Non-owner allowed to mint coins");
          });
  });
});
